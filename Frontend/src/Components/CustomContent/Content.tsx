...
import "./Content.scss";
import { Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TablePagination, useMediaQuery } from "@mui/material";
...
const Content: React.FC = () => {
  ...
  const fetchPolicyNetworkOnly = "network-only";
  const {organizationId,userName,userType} : any = useContext(TokenContext);
  ...
  const { data: countOrganizations, loading: loadingCountOrganizations } = useQuery(COUNT_ORGANIZATIONS, { fetchPolicy: fetchPolicyNetworkOnly });
  const { data: countUsers, loading: loadingCountUsers } = useQuery(COUNT_USERS, { fetchPolicy: fetchPolicyNetworkOnly });
  const { data: countJobPosts, loading: loadingCountJobPosts } = useQuery(COUNT_JOB_POSTS, { fetchPolicy: fetchPolicyNetworkOnly });
  const { data: countUserApplications, loading: loadingCountUserApplications } = useQuery(COUNT_USER_APPLICATIONS, {
    fetchPolicy: fetchPolicyNetworkOnly,
  });
              ...
              <TableContainer component={Paper}>
                <Table>
                  <TableHead className="tableHead">
                    <TableRow >
                      <TableCell align="center" width="10%" className="tableHeaderContent"  sx={{ fontSize: isSmallScreen ? '10px' : '14px' }}>
                        S.No
                      </TableCell>
